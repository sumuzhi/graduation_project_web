...
class index extends Component {
  ...
  sendApply = (apply_name, apply_id) => {
    ...
    applyToFriend({ apply_id, apply_name, username, number_id }).then(
      (result) => {
  ...
  handleCancel = (e) => {
    ...
    getFriendsList({ username, number_id }).then((result) => {
      if (result.status === 200) this.props.getFriendsLists(result.data);
    });
  };
  ...
  bypassApply = (apply_name, apply_id, e) => {
    ...
    operate_apply({ username, number_id, apply_id, apply_name, type }).then(
      (result) => {
        if (result.status === 200) {
          Toast.success(result.msg);
          this.getPreFriendsList();
  ...
  handleSubmit = (values) => {
    ...
    searchFriend(values).then((result) => {
      if (result.status !== 200) Toast.info(result.msg);
      else {
        this.setState({
              ...
              renderItem={(item) => (
                <List.Item
                  ...
                  extra={
                    <ButtonGroup
                      onClick={(e) =>
                        this.sendApply(item.username, item.number_id)
                      }
                      theme="borderless"
                    >
                      <Button>发送申请</Button>
                ...
                <List.Item
                  ...
                  extra={
                    <ButtonGroup theme="borderless">
                      <Button
                        onClick={(e) =>
                          this.bypassApply(item.username, item.number_id, e)
                        }
                      >
                        通过
                      </Button>
